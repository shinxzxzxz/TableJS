...
class HttpRequest {
    ...
    camelToTitleCase(inputString) {
        ...
        return inputString.replace(/[A-Z]/g, match => '-' + match)
    ...
    convertHeaders(headers) {
        ...
        if (headers) {
            Object.keys(headers).forEach(key => {
                const header = (key[0] === key[0].toUpperCase()) ? key : this.camelToTitleCase(key);
     ...
     */
    request() {
        let { url, timeout, dataType, success, error, beforeSend, afterSend } = this.config;
