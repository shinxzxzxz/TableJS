...
function preInitialize(instance) {
	...
	if (paginate) {
		if (paginate.local) {
			...
			if (limit) {
				if (!limit.hasLocalChangeLimitEventListener) {
					...
					limit.hasLocalChangeLimitEventListener = true;
				}
			}
			if (next) {
				if (!next.hasLocalClickNextEventListener) {
					next.addEventListener('click', () =>
						toPaginate({ table, ...next, to: 'local' })
					);
					next.hasLocalClickNextEventListener = true;
				}
			}
			if (previous) {
				if (!previous.hasLocalClickPreviousEventListener) {
					previous.addEventListener('click', () =>
						toPaginate({ table, ...previous, to: 'local' })
					);
					previous.hasLocalClickPreviousEventListener = true;
				}
			}
			if (search) {
				if (!search.hasLocalKeypressSearchEventListener) {
					search.addEventListener('keypress', (e) => {
						...
						let searchQuery = search.previousQuery;
						if (
					...
					search.hasLocalKeypressSearchEventListener = true;
				}
				if (!search.hasLocalInputSearchEventListener) {
					search.addEventListener('input', (e) => {
						if (e.target.value.length === 0) {
							search.previousQuery = DEFAULT.SEARCH;
							toSearch({
								table,
					...
					search.hasLocalInputSearchEventListener = true;
				}
			}
		}
		if (paginate.api) {
			...
			if (limit) {
				if (!limit.hasApiChangeLimitEventListener) {
					limit.addEventListener('change', (e) => {
						toLimit({
							table,
							limitSize: e.target.value,
					...
					limit.hasApiChangeLimitEventListener = true;
				}
			}
			if (next) {
				if (!next.hasApiClickNextEventListener) {
					next.addEventListener('click', () =>
						toPaginate({
							table,
							...next,
					...
					next.hasApiClickNextEventListener = true;
				}
			}
			if (previous) {
				if (!previous.hasApiClickPreviousEventListener) {
					previous.addEventListener('click', () =>
						toPaginate({
							table,
							...previous,
			...
			if (search) {
				if (!search.hasApiKeypressSearchEventListener) {
					search.addEventListener('keypress', (e) => {
						let searchValue = e.target.value;
						let searchQuery = search.previousQuery;
						if (
					...
					search.hasApiKeypressSearchEventListener = true;
				}
				if (!search.hasApiInputSearchEventListener) {
					search.addEventListener('input', (e) => {
						if (e.target.value.length === 0) {
							search.previousQuery = DEFAULT.SEARCH;
							toSearch({
					...
					search.hasApiInputSearchEventListener = true;
				}
			}
		}
	}
...
function processPagination(data = {}, total_rows) {
	...
	page = Math.min(Math.max(1, page), Math.round(total_rows / limit));
...
function propertiesListener({ elements, property, value }) {
	const { limit, search } = Object.entries(elements.paginate).reduce(
		(acc, [, values]) => {
	...
	switch (property) {
		case 'limit':
			search?.forEach((query) => (query.value = DEFAULT.SEARCH));
			break;
		case 'page':
			search?.forEach((query) => (query.value = DEFAULT.SEARCH));
			break;
...
function postInitialize(instance, isDisable = false) {
	...
	if (paginate) {
		if (paginate.local) {
			...
			// if (next) {
			//     next.disabled = isDisable;
			// }
			// if (previous) {
			//     previous.disabled = isDisable;
			// }
			// if (search) {
			//     search.disabled = isDisable;
			// }
		}
		if (paginate.api) {
			const { limit, next, previous, search } = paginate.api;
...
function toLimit({
	...
	let applyLimitation = () => {
		...
		let paginationInfo = {
			...
			total_page: paginationLength,
			start_item: paginationItems[0] + 1,
			end_item: paginationItems[paginationItemsLength - 1] + 1,
			total_rows: totalLength,
		};
...
function toPaginate({ table, as, to = 'local', output, instance }) {
	...
	let paginationInfo = {
		current_page: table.properties.page,
		total_page: paginationLength,
		start_item: paginationItems[0] + 1,
		end_item: paginationItems[paginationItemsLength - 1] + 1,
		total_rows: totalLength,
	...
	switch (to) {
		case 'local':
			...
			let result = rows.reduce((searched, row) => {
				...
				let isSearched = string.some((word) =>
					new RegExp(keyword, 'i').test(word)
				);
			...
			let paginationInfo = {
				current_page: 1,
				total_page: 1,
				start_item: Math.min(result, 1),
				end_item: dum ? Math.max(0, result - 1) : result,
				total_rows: dum ? rowLength - 1 : rowLength,
			};
